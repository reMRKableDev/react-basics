  ...
  */
  const showMyPets = (arrayOfPets) =>
    arrayOfPets.map((pet, index) => (
      <p key={index}>My favorite pet is called {pet}</p>
    ));

  const showMyNestedPets = (arrayOfPets) => {
    const eachPet = arrayOfPets.map((pet) => {
      const { id, type, name } = pet;

      return (
        <p key={id}>
          My favorite pet is a {type} called {name}. It has the id of {id}
        </p>
      );
    });
  ...
  return (
    <section className="App">
      ...
      <div className="column">
        ...
        <div className="row">
          <article className="react-element">
